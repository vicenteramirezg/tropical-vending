...
class MachineItemPrice(models.Model):
    machine = models.ForeignKey('core.Machine', on_delete=models.CASCADE, related_name='item_prices')
    product = models.ForeignKey('core.Product', on_delete=models.CASCADE, related_name='machine_prices')
    ...
    def __str__(self):
        return f"{self.product.name} at {self.machine} - ${self.price}"
